...
unsigned long jma_time[NB_TRY_REPET_LOOP];
...
void runTest(unsigned int startFreq, unsigned int targetFreq, unsigned int coreID)
{
...

   {
      ...
      do
      {
         ...
         fprintf(stdout,"index=%d time=%lu; low:%lu->high:%lu\n",niters,time,lowBoundTime,highBoundTime);
         fprintf(stdout,"======= round=%d freq  =======\n",round);
         for ( k = niters - 5 ; k <= niters ; k++ )
         {
             fprintf(stdout,"index=%03d; loop counter : %lu\n" ,k,jma_time[k]);
#ifdef _DUMP
              //writeDump(jma_time[k]);
#endif
         }
         for ( k = 0 ; k <= niters ; k++ )
         {             
         ...
         for ( k = 0 ; k < NB_VALIDATION_REPET ; k++ )
         { 
#ifdef _DUMP
             //writeDump(times[k]);
#endif
         }
         if ( validateBenchTime > validateHighBoundTime  || validateBenchTime < validateLowBoundTime )
         {
            fprintf(stdout,"validateBenchTime=%f; low:%lu->high:%lu\n",validateBenchTime,validateLowBoundTime,validateHighBoundTime);
            validated = 0;
            setFreq(coreID,startFreq);
            waitCurFreq(coreID,startFreq);
            break; //jma
...
void *thfn(void *arg)
{
   ...
   if (sched_setscheduler(getpid(), SCHED_BATCH, &sp))
   {
      perror("setscheduler-thfn background");
   }
...
int main(int argc, char** argv)
{
 ...
 /*  if (sched_setscheduler(getpid(), SCHED_FIFO, &sp))
   {
      perror("setscheduler-main background");
   }
*///jma
